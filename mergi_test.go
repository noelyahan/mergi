...
func ExampleCrop_croping() {
	...
	err = mergi.Export(io.NewFileExporter(resultImage, "result.jpg"))
	if err != nil {
...
func ExampleWatermark_watermarking() {
	// Get the image content by passing image path url or file path
	imgOriginal, err := mergi.Import(io.NewFileImporter("testdata/tiger-2320819_960_720.jpg"))
	if err != nil {
		log.Fatalf("failed to open: %s", err)
	}
	// Get the image content by passing image path url or file path
	imgWatermark, err := mergi.Import(io.NewFileImporter("./testdata/mergi_logo_watermark.png"))
	if err != nil {
		log.Fatalf("failed to open: %s", err)
	}
...
func ExampleImport_importing() {
	// Get the image content by passing image path url or file path
	imgFromLocal, err := mergi.Import(io.NewFileImporter("testdata/tiger-2320819_960_720.jpg"))
	if err != nil {
		log.Fatalf("failed to load via local: %s", err)
	}
	// Get the image content by passing image path url or file path
	// Reference: https://pixabay.com/en/woman-old-senior-female-elderly-1031000/
	imageFromURL, err := mergi.Import(io.NewURLImporter("https://cdn.pixabay.com/photo/2015/11/07/11/17/woman-1031000__340.jpg"))
	if err != nil {
...
func ExampleAnimate_animating() {
	// Get the image content by passing image path url or file path
	img1, err := mergi.Import(io.NewFileImporter("testdata/avocado-3210885_960_720.jpg"))
	if err != nil {
		log.Fatalf("failed to load via local: %s", err)
	}
	img2, err := mergi.Import(io.NewFileImporter("testdata/cherry-3074284_960_720.jpg"))
	if err != nil {
		log.Fatalf("failed to load via local: %s", err)
	}
	...
	mergi.Export(io.NewAnimationExporter(animation, "result.gif"))
}
